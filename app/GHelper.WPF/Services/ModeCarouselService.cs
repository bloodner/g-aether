    ...
    public static class ModeCarouselService
    {
        ...
        public static void Show(string category, List<ModeCarouselItem> items, int selectedIndex)
        {
            Application.Current?.Dispatcher.Invoke(() =>
            {
                ...
                bool needsShow = !_window!.IsVisible;

                if (categoryChanged || needsShow)
                    RebuildTiles();
                ...
                if (needsShow)
                {
                    ...
                    // Position after show so DPI and actual size are available
                    PositionWindow();
                    ...
                    _window.BeginAnimation(UIElement.OpacityProperty,
        ...
        private static void PositionWindow()
        {
            ...
            // Use actual rendered size if available, otherwise measure
            double winW = _window.ActualWidth;
            double winH = _window.ActualHeight;
            if (winW == 0 || winH == 0)
            {
                _window.Measure(new Size(double.PositiveInfinity, double.PositiveInfinity));
                _window.Arrange(new Rect(_window.DesiredSize));
                winW = _window.DesiredSize.Width;
                winH = _window.DesiredSize.Height;
            }

            var screen = System.Windows.Forms.Screen.PrimaryScreen;
            if (screen != null)
            {
                // Get DPI from the carousel window itself (available after Show)
                var source = PresentationSource.FromVisual(_window);
                double dpiScale = source?.CompositionTarget?.TransformFromDevice.M11 ?? 1.0;

                // Fallback if source isn't ready
                if (source == null)
                {
                ...
                double screenW = screen.Bounds.Width * dpiScale;
                double screenH = screen.Bounds.Height * dpiScale;
                double screenLeft = screen.Bounds.Left * dpiScale;
                double screenTop = screen.Bounds.Top * dpiScale;

                _window.Left = screenLeft + (screenW - winW) / 2;
                _window.Top = screenTop + (screenH - winH) / 2;
            }
        }
